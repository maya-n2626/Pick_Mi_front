...
export async function fetchAllUsers() {
  try {
    ...
    container.querySelectorAll(".admin-delete-user").forEach(
      (btn) =>
        (btn.onclick = async () => {
...
export async function fetchAllNotes() {
  try {
    ...
    container.innerHTML = notes
      ...
      .join("");
    // רישום מאזינים למחיקה
    container.querySelectorAll(".admin-delete-note").forEach(
      (btn) =>
        (btn.onclick = async () => {
          if (!confirm("למחוק פתק זה?")) return;
          await apiFetch(`/api/admin/notes/${btn.dataset.id}`, {
            method: "DELETE",
          });
          fetchAllNotes();
        }),
